...
│   ├── extension.ts                   # Activation entry point, commands, status bar
│   ├── panel/
│   │   ├── main/
│   │   │   ├── MainPanel.ts           # Main webview panel manager (message routing)
│   │   │   └── handlers/              # 5 handler modules (REST, GraphQL, gRPC, SOAP, Realtime)
│   │   └── sidebar/
│   │       └── WelcomeViewProvider.ts # Activity bar welcome view
│   ├── http/
│   │   ├── request-executor.ts        # Core HTTP execution (Axios) + script runner
│   │   └── timing-agent.ts           # DNS/TCP/TLS timing breakdown
│   ├── grpc/
│   │   ├── grpc-executor.ts           # gRPC execution (@grpc/grpc-js)
│   │   ├── grpc-reflection.ts         # gRPC server reflection client
│   │   └── proto-loader.ts            # .proto file loader + service resolution
│   ├── soap/
│   │   ├── soap-executor.ts           # SOAP execution (soap library)
│   │   ├── wsdl-service.ts            # WSDL parsing + operation discovery
│   │   ├── soap-envelope-builder.ts   # SOAP envelope construction
│   │   ├── ws-security.ts             # WS-Security (UsernameToken, Timestamp, Signature)
│   │   └── soapui-importer.ts         # SOAPUI project importer
│   ├── mcp/
│   │   ├── mcp-client.ts              # MCP client orchestration
│   │   ├── mcp-stdio-transport.ts     # stdio-based MCP transport
│   │   ├── mcp-http-transport.ts      # HTTP-based MCP transport
│   │   └── mcp-types.ts              # MCP protocol types
│   ├── mock/
│   │   ├── mock-server-manager.ts     # Multi-protocol mock server management
│   │   ├── mock-http-server.ts        # HTTP/REST mock (Express)
│   │   ├── mock-graphql-server.ts     # GraphQL mock
│   │   ├── mock-grpc-server.ts        # gRPC mock
│   │   ├── mock-soap-server.ts        # SOAP mock
│   │   ├── mock-ws-server.ts          # WebSocket mock
│   │   ├── mock-sse-server.ts         # SSE mock
│   │   ├── mock-socketio-server.ts    # Socket.IO mock
│   │   ├── mock-mqtt-server.ts        # MQTT mock (Aedes broker)
│   │   ├── mock-graphql-schema.ts     # GraphQL schema resolver
│   │   └── mock-types.ts             # Shared mock types
│   ├── ai/
│   │   ├── ai-executor.ts             # AI request execution (LLM calls)
│   │   ├── ai-providers.ts            # Multi-provider LLM support
│   │   └── ai-types.ts               # AI type definitions
│   ├── services/
│   │   ├── collection-runner.ts       # Sequential collection execution
│   │   ├── postman-importer.ts        # Postman Collection v2.1 importer
│   │   ├── openapi-importer.ts        # OpenAPI/Swagger 3.x + 2.x importer
│   │   ├── har-importer.ts            # HAR (HTTP Archive) importer
│   │   ├── bruno-importer.ts          # Bruno `.bru` file importer
│   │   ├── oauth2.ts                  # OAuth 2.0 flow handler
│   │   ├── script-resolver.ts         # Variable resolution engine
│   │   ├── script-runtime/            # JavaScript sandbox + dk API
│   │   ├── variables/                 # Variable substitution utilities
│   │   ├── debugger/                  # Script debugger backend
│   │   └── import-types.ts           # Shared import type definitions
│   └── storage/
│       └── db.ts                      # SQLite init, migrations, CRUD, KV ops (sql.js)
│
├── webview-ui/                        # React Webview (Vite 6 + Tailwind v4 + Zustand 5)
│   ├── index.html
│   ├── vite.config.ts
│   └── src/
│       ├── main.tsx                   # Entry point
│       ├── App.tsx                    # Root orchestrator (protocol routing)
│       ├── index.css                  # Indigo theme tokens + Tailwind
│       ├── vscode.ts                  # postMessage bridge (typed)
│       ├── colors/
│       │   └── daakia-colors.ts       # Centralized color system (CSS vars + JS exports)
│       ├── icons/
│       │   └── daakia-icons.tsx       # Centralized SVG icon components
│       ├── store/                     # Zustand stores (11 total)
│       │   ├── tabs-store.ts          # Open tabs, active tab, tab state
│       │   ├── collections-store.ts   # Collections tree, CRUD
│       │   ├── env-store.ts           # Environments, global vars, resolution
│       │   ├── toast-store.ts         # Toast notifications
│       │   ├── sidebar-data-store.ts  # Sidebar panel state
│       │   ├── debug-store.ts         # Script debugger state
│       │   ├── devtools-store.ts      # DevTools panel state
│       │   ├── ui-state-store.ts      # UI preferences (theme, layout)
│       │   ├── mock-store.ts          # Mock server state
│       │   ├── ai-providers-store.ts  # AI provider configuration
│       │   └── url-suggestions-store.ts # URL autocomplete
│       ├── hooks/                     # Custom React hooks
│       │   ├── useClickOutside.ts     # Click-outside detection
│       │   └── ...
│       ├── utils/
│       │   ├── code-generator.ts      # 12-language code snippet generator
│       │   ├── curl-parser.ts         # cURL command parser
│       │   └── ...
│       ├── services/                  # Webview-side service modules
│       │   ├── collections/
│       │   ├── request/
│       │   └── ...
│       └── components/
│           ├── shared/                # Cross-protocol reusable components
│           │   ├── controls/          # KeyValueTable, FormDataTable, PillTabs,
│           │   │                        SplitButton, StyledDropdown, Checkbox,
│           │   │                        HighlightedInput, DurationInput, ResizablePanel
│           │   ├── modals/            # ConfirmDialog, SaveRequestModal, NewItemModal,
│           │   │                        GenerateCodeModal, ImportCurlModal, RunCollectionModal
│           │   ├── menus/             # ContextMenu (custom right-click), StyledDropdown
│           │   ├── editors/           # AuthEditor, ScriptsEditor, CodeEditor (Monaco),
│           │   │                        JsonEditor
│           │   ├── display/           # Toast, InfoPopup, MethodBadge, StatusBadge
│           │   ├── debugger/          # DebugCodeView, DebugHud, DebugPanel,
│           │   │                        DebugNetworkSection, RunAndDebugPanel
│           │   └── devtools/          # DevToolsPanel, ConsoleTab, NetworkTab,
│           │                            TimelineTab, PerformanceTab
│           ├── tabs/                  # TabBar (shared tab system, drag-drop, context menu)
│           ├── sidebar/               # AppSidebar, SettingsPanel, LlmProviderSettings
│           ├── rest/                  # REST API components
│           │   ├── request/           # UrlBar, RequestConfig
│           │   ├── response/          # ResponsePanel, BodyView, HeadersView,
│           │   │                        CookiesView, TimelineView, TestResults
│           │   └── sidebar/           # CollectionsPanel, HistoryPanel, EnvironmentsPanel
│           ├── graphql/               # GraphQL components
│           │   ├── GraphQLPanel.tsx, GraphQLEditor.tsx, GraphQLUrlBar.tsx
│           │   ├── GraphQLSchemaPanel.tsx, GraphQLDocumentationPanel.tsx
│           │   ├── GraphQLResponse.tsx, GraphQLSubscription.tsx
│           │   └── GraphQLQueryTabs.tsx
│           ├── grpc/                  # gRPC components
│           │   ├── GrpcPanel.tsx, GrpcUrlBar.tsx
│           │   ├── GrpcProtoManager.tsx, GrpcMethodSelector.tsx
│           │   └── GrpcRequestConfig.tsx, GrpcResponsePanel.tsx
│           ├── soap/                  # SOAP components
│           │   ├── SoapPanel.tsx, SoapUrlBar.tsx
│           │   ├── SoapWsdlBrowser.tsx, SoapOperationSelector.tsx
│           │   ├── SoapFormEditor.tsx, SoapRequestConfig.tsx
│           │   ├── SoapResponsePanel.tsx, SoapHeadersEditor.tsx
│           │   ├── SoapAttachments.tsx, SoapAssertions.tsx
│           │   └── SoapWsdlImport.tsx
│           ├── websocket/             # WebSocket / Realtime components
│           │   ├── RealtimeProtocolSelector.tsx
│           │   ├── ws/                # WebSocket client
│           │   ├── sse/               # SSE client
│           │   ├── sio/               # Socket.IO client
│           │   └── mqtt/              # MQTT client
│           ├── mcp/                   # MCP client components
│           │   ├── McpPanel.tsx, McpUrlBar.tsx
│           │   ├── McpRequestTabs.tsx, McpResponsePanel.tsx
│           │   └── tabs/              # Tools, Resources, Prompts tabs
│           └── mock/                  # Mock Server components
│               ├── MockServerPanel.tsx, ServerList.tsx
│               ├── ServerDetail.tsx, RouteCard.tsx
│               ├── MockLogPanel.tsx
│               └── configs/           # Protocol-specific mock configs
│
├── images/
│   ├── daakia-icon.png                # Extension + Marketplace icon (128×128)
│   ├── daakia-side.svg                # Activity bar icon
│   ├── daakia-icons.json              # Icon font mapping
│   └── daakia-icons.woff              # Icon font (product icons)
│
├── scripts/
│   ├── generate-icon-font.js          # SVG → icon font generator
│   └── zip_daakia_latest.js           # Backup zip script
│
├── plan/                              # Project planning & documentation
│   ├── todo.md                        # Master task tracker (Sprint 1-4)
│   ├── 01-overview.md                 # Vision & tech stack
│   ├── 02-features.md                 # Complete feature spec
│   ├── 03-project-structure.md        # Directory tree + architecture
│   ├── 04-ui-wireframes.md            # UI layouts & mockups
│   ├── 05-ai-agents.md                # AI agent pool definitions
│   ├── 06-mock-server.md              # Mock server specification
│   ├── 07-implementation-roadmap.md   # Sprint roadmap
│   ├── 08-extension-manifest.md       # package.json contribution points
│   ├── 09-data-models.md              # TypeScript type definitions
│   ├── 10-component-specs.md          # Component behavior specs
│   ├── 11-sqlite-database.md          # Database schema & setup
│   ├── ai_protocol.md                 # AI integration protocol
│   ├── grpc.md                        # gRPC implementation spec
│   ├── mcp_protocol.md                # MCP client spec
│   ├── sprint2-extension-feature.md   # Sprint 2 detailed feature spec
│   ├── wiremock-daakia.md             # WireMock integration
│   ├── changes/                       # Session change logs (50+)
│   ├── test/                          # Manual test suites
│   │   ├── sprint1/tests.md           # Foundation tests
│   │   ├── sprint2/tests.md           # Request builder tests
│   │   ├── sprint3/tests.md           # Advanced collections tests
│   │   ├── sprint5/tests.md           # Protocol restructure tests
│   │   ├── sprint5/debugger-tests.md  # Script debugger tests
│   │   └── sprint6/tests.md           # SOAP protocol tests
│   └── bruno/                         # Bruno integration docs
│
├── .github/
│   ├── agents/
│   │   └── MrDaakia.agent.md          # AI agent definition for development
│   └── instructions/
│       └── daakia-conventions.instructions.md  # Coding conventions
│
