...
/**
 * Category base for: ButtonView, IconButtonView, SplitButtonView, and any future button component.
 * Falls back to DuiProvider size when no local `size` prop is given.
 */
export function useButtonBase(sizeProp?: DuiSize): ButtonBaseConfig {
  const { size } = useDui();
