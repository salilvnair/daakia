...
const SIDEBAR_GROUPS: SidebarGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'copybutton',         label: 'CopyButtonView',         icon: <CopyIcon size={13} /> },
...
const PANELS: Record<CategoryId, { title: string; desc: string; content: React.ReactNode; vars?: LiveColorVar[]; code?: string; noExamplesHeader?: boolean }> = {
  ...
  aibutton:          { title: 'AIButtonView',           desc: 'AI action button — generate · fuzz · explain · fix · ask · suggest — loading state.', vars: VARS_AIBTN,   content: <AIButtonPanel />,          code: `function Preview() {\n  const [loading, setLoading] = useState(false);\n  const handleClick = () => {\n    setLoading(true);\n    setTimeout(() => setLoading(false), 2000);\n  };\n  return (\n    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-start' }}>\n      <AIButtonView action="generate" onClick={handleClick} loading={loading} />\n      <AIButtonView action="explain"  onClick={handleClick} loading={loading} />\n      <AIButtonView action="fix"      onClick={handleClick} loading={loading} />\n    </div>\n  );\n}` },
  ...
  collapsiblesection: { title: 'CollapsibleSectionView', desc: 'Expandable section with chevron toggle, colored chip title, count badge, and a right-side action slot. Used in debugger sections (Variables, Watch, Call Stack, Breakpoints, Network).', vars: VARS_ACCENT, content: <CollapsibleSectionPanel />, code: `function Preview() {\n  const [expanded, setExpanded] = useState(true);\n  return (\n    <CollapsibleSectionView\n      title="Variables"\n      expanded={expanded}\n      onToggle={() => setExpanded(v => !v)}\n      accentColor="var(--color-debug-key)"\n      badge={3}\n    >\n      <div style={{ padding: '6px 8px', fontSize: 12, color: 'var(--color-text-secondary)' }}>\n        x = 42, y = "hello", active = true\n      </div>\n    </CollapsibleSectionView>\n  );\n}` },
  jsontree:           { title: 'JsonTreeView', desc: 'Recursive JSON / object / array value tree with VS Code-style token colors. Handles strings, numbers, booleans, null, nested objects, and arrays. Expandable nodes with configurable default depth.', vars: VARS_ACCENT, content: <JsonTreeViewPanel />, code: `<JsonTreeView\n  data={{ user: { id: 1, name: 'Alice', roles: ['admin'] } }}\n  defaultExpandDepth={2}\n/>` },
  logentry:           { title: 'ExpandableLogEntryView', desc: 'Expandable log row with icon, title, colored badge, timestamp chip, and chevron. Used in Network Logs view for dk.sendRequest() call traces. Composable — any content in the expanded section.', vars: VARS_ACCENT, content: <ExpandableLogEntryPanel />, code: `<ExpandableLogEntryView\n  icon={<ArrowUpRightIcon size={13} />}\n  title="Request Sent"\n  badge="POST"\n  badgeColor="var(--color-method-post)"\n  timestamp={Date.now()}\n>\n  <div>POST https://api.example.com/v2/users</div>\n</ExpandableLogEntryView>` },
  copybutton:         { title: 'CopyButtonView', desc: 'Icon-only button that copies text to clipboard. Shows CopyIcon → CheckIcon swap for 1500ms on success. Supports DUI size tokens (xxs → xl) — controls both button dimensions and icon size via useButtonBase.', vars: VARS_ACCENT, content: <CopyButtonPanel />, code: `<CopyButtonView text="some text to copy" />\n<CopyButtonView text="token" size="xs" accentColor="var(--color-primary)" />\n<CopyButtonView text="large" size="lg" />` },
  markdownview:       { title: 'MarkdownView', desc: 'Renders Markdown (GFM) with syntax-highlighted code blocks (highlight.js), inline code, tables, task lists, blockquotes, and copy-buttons on each code block. Uses singleton marked config — safe to mount multiple times.', vars: VARS_ACCENT, content: <MarkdownViewPanel />, code: `<MarkdownView content={"## Hello\\n\\nInline \`code\` and a code block:\\n\\n\`\`\`ts\\nconst x = 1;\\nconsole.log(x);\\n\`\`\`\\n\\n- item one\\n- item two"} />` },
