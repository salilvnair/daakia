...
const PANELS: Record<CategoryId, { title: string; desc: string; content: React.ReactNode; vars?: LiveColorVar[]; code?: string; noExamplesHeader?: boolean }> = {
  ...
  copybutton:         { title: 'CopyButtonView', desc: 'Icon-only button that copies text to clipboard. Shows CopyIcon → CheckIcon swap for 1500ms on success. No store or postMsg deps — pure inline-style DUI component. Configurable icon size and accent color.', vars: VARS_ACCENT, content: <CopyButtonPanel />, code: `<CopyButtonView text="some text to copy" />\n<CopyButtonView text="token" accentColor="var(--color-primary)" size={16} />` },
