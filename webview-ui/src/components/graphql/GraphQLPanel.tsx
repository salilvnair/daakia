...
import { GraphQLUrlBar } from './GraphQLUrlBar';
import { GraphQLEditor } from './GraphQLEditor';
import { GraphQLResponse } from './GraphQLResponse';

/**
 * GraphQL Panel — main panel shown when protocol=graphql.
 * Layout: URL bar → vertical split (editor top / response bottom) — same as REST.
 */
export function GraphQLPanel() {
  const activeTab = useTabsStore(s => s.tabs.find(t => t.id === s.activeTabId));

  // Vertical split state (same pattern as REST in App.tsx)
  const storedSplit = useUiStateStore(s => s.panelHeights['split.gql.main']);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    ...
    setIsDragging(false);
    useUiStateStore.getState().setHeight('split.gql.main', splitPercent);
  }, [splitPercent]);

  const handleDoubleClick = useCallback(() => {
    setSplitPercent(50);
    setFocusedPanel(null);
    useUiStateStore.getState().setHeight('split.gql.main', 50);
  }, []);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      {/* Vertical split: editor (top) / response (bottom) */}
      <div ref={containerRef} className="flex-1 flex flex-col min-h-0 relative">
        {/* Top: Query editor + variables/headers/auth */}
        <div
        ...
        {/* Splitter (horizontal bar — same style as REST) */}
        <div
        ...
        >
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[3px] rounded-full transition-all duration-150 ${
            isDragging
              ? 'w-[80px] bg-[var(--color-protocol-graphql)]'
              : 'w-[44px] bg-[var(--color-surface-border)] group-hover:bg-[var(--color-protocol-graphql)] group-hover:w-[80px]'
          }`} />
          {/* Tooltip */}
