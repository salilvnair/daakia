...
import { GrpcRequestConfig } from './GrpcRequestConfig';
import { GrpcResponsePanel } from './GrpcResponsePanel';

const ACCENT = 'var(--color-protocol-grpc)';

/**
 * GrpcPanel — main panel shown when protocol=grpc.
...
export function GrpcPanel() {
  ...
  const storedSplit = useUiStateStore(s => s.panelHeights['split.grpc.main']);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    ...
    setIsDragging(false);
    useUiStateStore.getState().setHeight('split.grpc.main', splitPercent);
  }, [splitPercent]);

  const handleDoubleClick = useCallback(() => {
    setSplitPercent(50);
    setFocusedPanel(null);
    useUiStateStore.getState().setHeight('split.grpc.main', 50);
  }, []);
