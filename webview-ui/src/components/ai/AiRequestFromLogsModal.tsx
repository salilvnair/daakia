...
192.168.1.3 - - [08/Jun/2026:10:30:10] "DELETE /api/v1/users/42 HTTP/1.1" 204 0`;
  ...
  return (
    <ModalView
      open
      onClose={onClose}
      title="Request from Logs"
      subtitle="Paste server access logs → AI extracts requests → creates collection"
      size="lg"
      headerColor={ACCENT}
      headerIcon={
      ...
      footerLeft={
        result && !loading ? (
          <ButtonView
            ...
            disabled={imported}
            onClick={importAsCollection}
          >
            {imported ? <><CheckIcon size={12} style={{ marginRight: 4 }} />Imported!</> : <><SparkleIcon size={11} style={{ marginRight: 4 }} />Import as Collection</>}
          </ButtonView>
        ) : undefined
      }
      footerRight={
        <AIButtonView
          label={loading ? 'Extracting…' : 'Extract Requests'}
          size="md"
          accentColor={ACCENT}
