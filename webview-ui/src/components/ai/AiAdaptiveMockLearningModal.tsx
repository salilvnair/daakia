...
export function AiAdaptiveMockLearningModal({ onClose, onApply }: Props) {
  ...
  return (
    <ModalView
      ...
      footerRight={
        result && onApply ? (
          <ButtonView size="md" variant="primary" accentColor="var(--color-success)" onClick={() => { onApply(result); onClose(); }}>
            Apply Rules
          </ButtonView>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {status === 'idle' && (
          <div className="flex gap-2 items-center">
            ...
            <span className="text-[11px]" style={{ color: 'var(--color-text-muted)' }}>or paste traffic sample below and click Analyze</span>
          </div>
        )}

        {status === 'recording' && (
          <div className="flex flex-col gap-2">
            <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--color-surface-border)' }}>
            ...
            <div className="flex justify-between items-center">
              ...
              <ButtonView size="md" variant="primary" accentColor={ACCENT} onClick={handleAnalyze}>Stop & Analyze</ButtonView>
            </div>
          </div>
        )}
        ...
        {(status === 'idle' || status === 'done') && (
          <div className="flex flex-col gap-2">
            ...
            <MultilineInputView
              value={trafficSample}
              onChange={e => setTrafficSample(e.target.value)}
        ...
        {status === 'processing' && !result && (
          <div className="flex items-center gap-2 py-3 justify-center">
            <span className="inline-block w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: ACCENT }} />
        ...
        {result && (
          <div className="flex flex-col gap-2">
            ...
            <div className="rounded border p-3 overflow-y-auto" style={{ maxHeight: 280, borderColor: 'var(--color-surface-border)', background: 'var(--color-surface)' }}>
