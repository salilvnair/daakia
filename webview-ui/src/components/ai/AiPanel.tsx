...
import { AiUrlBar } from './AiUrlBar';
import { AiRequestTabs } from './AiRequestTabs';
import { AiConversationPanel } from './AiConversationPanel';

/**
 * AiPanel — main panel shown when protocol=ai.
 * Layout: URL bar → vertical split (request config top / conversation bottom).
 */
export function AiPanel() {
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    ...
    setIsDragging(false);
    useUiStateStore.getState().setHeight('split.ai.main', splitPercent);
  }, [splitPercent]);

  const handleDoubleClick = useCallback(() => {
    setSplitPercent(50);
    setFocusedPanel(null);
    useUiStateStore.getState().setHeight('split.ai.main', 50);
  }, []);
  ...
  const handleFocus = useCallback((panel: 'request' | 'response') => {
    ...
    useUiStateStore.getState().setHeight('split.ai.main', target);
  }, [isDragging]);
      ...
      >
        ...
        >
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[3px] rounded-full transition-all duration-150 ${
            isDragging
              ? 'w-[80px] bg-[var(--color-protocol-ai)]'
              : 'w-[44px] bg-[var(--color-surface-border)] group-hover:bg-[var(--color-protocol-ai)] group-hover:w-[80px]'
          }`} />
