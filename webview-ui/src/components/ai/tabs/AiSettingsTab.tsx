...
/**
 * AiSettingsTab — Model parameters using DUI components throughout.
 */
  ...
  return (
    <div className="flex flex-col px-4 py-3 gap-2.5 overflow-auto">
      ...
      <div className="flex items-center">
        ...
        <ToggleSwitchView
          checked={stream}
          onChange={(v) => updateSetting('stream', v)}
          size="sm"
          accentColor={ACCENT}
          label={stream ? 'Real-time streaming' : 'Wait for full response'}
          labelPosition="right"
        />
      </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {stopSequences.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {stopSequences.map((seq: string, i: number) => (
                <span
                  key={i}
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!activeTab) return;
                      const updated = stopSequences.filter((_: string, idx: number) => idx !== i);
                      updateTab(activeTab.id, { aiSettings: { ...settings, stopSequences: updated }, dirty: true });
                    }}
