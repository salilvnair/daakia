...
const SCREENS: ScreenSpec[] = [
  { id: 'graphql-query', label: 'GraphQL — Query',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: BASE_PATCH }, { action: 'click', selector: 'button[data-tab="query"]' }, { action: 'wait', ms: 1800 } ] },
  { id: 'graphql-variables', label: 'GraphQL — Variables',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: BASE_PATCH }, { action: 'click', selector: 'button[data-tab="variables"]' }, { action: 'wait', ms: 1800 } ] },
  { id: 'graphql-headers', label: 'GraphQL — Headers',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: { ...BASE_PATCH, headers: [{ id: 'h1', key: 'Authorization', value: 'Bearer {{token}}', enabled: true }] } }, { action: 'click', selector: 'button[data-tab="headers"]' }, { action: 'wait', ms: 600 } ] },
  { id: 'graphql-authorization', label: 'GraphQL — Authorization',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: { ...BASE_PATCH, authType: 'bearer', authData: { ...BASE_PATCH.authData, token: 'eyJhbGciOiJIUzI1NiJ9.mock_token' } } }, { action: 'click', selector: 'button[data-tab="authorization"]' }, { action: 'wait', ms: 600 } ] },
  { id: 'graphql-scripts', label: 'GraphQL — Scripts',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: { ...BASE_PATCH, preRequestScript: '// Refresh token before sending', postResponseScript: 'dk.test("No GraphQL errors", () => {\n  dk.expect(dk.response.json().errors).toBeUndefined();\n});' } }, { action: 'click', selector: 'button[data-tab="scripts"]' }, { action: 'wait', ms: 1800 } ] },
  { id: 'graphql-subscription', label: 'GraphQL — Subscription',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: BASE_PATCH }, { action: 'click', selector: 'button[data-tab="subscription"]' }, { action: 'wait', ms: 600 } ] },
  { id: 'graphql-response', label: 'GraphQL — Response',
    ...
    directives: [ { action: 'closeAllTabs' }, { action: 'addTab', patch: RESPONSE_PATCH }, { action: 'wait', ms: 1800 } ] },
];
